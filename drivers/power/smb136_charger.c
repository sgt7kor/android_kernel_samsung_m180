...
int smb136_get_connect_status(void) 
{
	struct smb136_chg_data * chg = smb136_chg;
	u8 data = 0;
	u32 addr = 0;
	...
	/* Read Charger OVLO and UVLO */
	smb136_i2c_read(chg->client, 0x37, &data);		
	//printk("SMB136 addr : 0x37 data : 0x%x ::: Charger OVLO =0x%x, Charger UVLO=0x%x\n", data,(data&0x04)?1:0, (data&0x02)?1:0);
	
	if (data & 0x6) /* if UVLO or OVLO case, charging should be stopped.*/
		return 0;
	else
		return 1;
}
...
static int smb136_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chg->chgdev->get_connection_status = smb136_get_connect_status;
